...
**v0.2 change:** Adds explicit support for existing DDR GraphQL database authorities and archive-to-LLM evidence presets. Authorities may assist entity resolution, transparent retrieval and optional contextualisation, but must remain inspectable and distinct from source-document evidence.
...
- explicit separation of source-document evidence, archive/catalogue metadata, database authorities, corpus-control metadata and generated inference.
...
15. existing GraphQL database-authority presets and schemas, including staff, jobs/projects, fonds, publication types, students and critical/controlled categories;
16. existing archive-to-LLM evidence-surface presets and any authority/entity linkage already exposed.
...
1. retrieved source-document evidence;
2. archive/catalogue metadata;
3. database-authority assertions;
4. corpus-control and provenance metadata;
5. model-generated inference;
6. missingness or uncertainty;
7. researcher assessment.

### 2.5 Archive/database authorities

The live archive GraphQL API exposes structured authorities and presets that may include:

- DDR staff;
- DDR staff registry, roles and tenure;
...
- project outcomes;
- epistemic stances;
- other controlled categories present in the live schema.

These authorities may be used only through explicit, inspectable roles:

- `entity_resolution`;
- `retrieval_filter`;
- `controlled_query_expansion`;
- `structural_context`;
- `corroborative_check`.

Authority assertions are **not** source-document evidence.

Do not silently concatenate authority values with OCR/source text.

Do not automatically inject every authority field into Granite.

Interpretative authorities such as epistemic stance, methodology, beneficiary audience or project outcome may encode prior institutional or researcher judgement. Their influence must be visible whenever they affect retrieval or inference.

Where authority context is supplied to Granite, record:

```text
authority_context_used
authority_sources
authority_fields_supplied
authority_values_snapshot
authority_context_version
```

The experiment must be able to distinguish a relationship surfaced from documentary traces from a relationship already encoded by archive authorities.
...
Archive GraphQL
    ...
    └── corpus-control / provenance metadata
            ↓
ML eligibility + permitted page scope
            ↓
PDF/TIFF source
            ↓
Docling ingestion
...
PostgreSQL FTS query
      + transparent authority assistance where configured
            ↓
ranked retrieved chunks
      + optional labelled authority context
            ↓
context assembly
            ↓
...
class Document:
    ...
    authority_links_json: dict
    metadata_roles_version: str | None
...
### 6.3 Authority context

Authority data may be cached or snapshotted only as needed for reproducibility. Match existing repository conventions.
...
class AuthorityContext:
    ...
    fields_json: dict
    role: str
    source_query_or_preset: str | None
    snapshot_version: str | None
    retrieved_at: datetime
```

Do not duplicate the whole authority database locally unless the existing architecture requires it.

Store enough information to reproduce which authority assertions influenced a run.

### 6.4 Experiment prompt
...
## 8. Authority-layer requirements

### 8.1 Discover and map live authorities

Inspect the actual GraphQL presets/schema and document which authorities are available.

At minimum investigate:

- DDR staff;
- staff registry / roles / tenure;
- job/project register;
- fonds;
- publication type;
- students;
- DDR period;
...
Do not invent fields or assume presets are stable without recording the live schema/query used.

### 8.2 Role mapping

Create a documented authority-role matrix with columns equivalent to:
...
### 8.3 Authority use in retrieval

Permitted:

- canonical person/project identifiers;
- name variants;
- role/tenure filters;
- job/project numbers;
- fonds and publication-type filters;
- controlled methodology/theme variants;
- archive-specific spelling/acronym expansion.

Do not allow opaque model-generated authority expansion for the main experiment.

Record every authority-derived expansion or filter with the run.

### 8.4 Authority use in Granite context

Authority context must be optional, bounded and clearly labelled.

Example:

```text
[ARCHIVE / DATABASE AUTHORITY CONTEXT]
SOURCE: DDR staff registry
PERSON: L. Bruce Archer
ROLE: ...
TENURE: ...
...
The system prompt must tell Granite that authority context is structured archive information, not quotation from the source document.

### 8.5 Interpretative-risk authorities
...
as potentially interpretative rather than neutral facts.

Do not use them to validate their own inferred conclusions.

Where useful, support document-only versus authority-assisted sensitivity comparisons using identical retrieved source chunks.

### 8.6 Authority reproducibility

When authority data affects a run, persist:

- source/preset/query identifier;
- authority IDs;
- fields and values supplied;
- role in the run;
- snapshot/version or retrieval timestamp;
- any conflicts or missing mappings.
...
### 10.2 Retrieval service contract
...
If authority context is enabled for a run, place it in a separate labelled block before or after source-document evidence. Never merge authority assertions into `TEXT:`.

Context assembly must record whether the run is:

- `document_only`;
- `document_plus_authority_context`.

The exact authority sources and fields supplied must be saved.
...
Distinguish SOURCE DOCUMENT EVIDENCE from ARCHIVE / DATABASE AUTHORITY CONTEXT.
Do not present authority metadata as if it were quoted from a source document.
...
- authority claims cited as authority context can be traced to the recorded authority source/snapshot;
- authority context is never validated as source-document evidence.
...
Authority absence is also not historical absence. A missing person/project in an authority table may reflect incomplete structured description, not non-existence or non-participation.

---

## 17. Experiment service
...
async def run_archival_experiment(
    ...
    # 2. Resolve configured authority context / transparent expansions.
    # 3. Execute transparent retrieval.
    # 4. Generate retrieval diagnostics.
    # 5. Assemble bounded document context plus optional labelled authority context.
    # 6. Call Granite.
    # 7. Parse structured response.
    # 8. Validate source and authority provenance.
    # 9. Store run, evidence, authority context and diagnostics.
    # 10. Return saved run.
    pass
```
...
- researcher assessment;
- authority context used, its sources, roles, fields and snapshot/version.
...
- authority source/preset;
- authority type;
- authority identifiers;
- fields supplied;
- role in retrieval or inference;
- snapshot/version;
- a clear statement that authority context is not source-document evidence.

Hide or collapse this panel for document-only runs.
...
- authority-derived query expansion logging;
- authority context labelling;
- separation of authority assertions from source-document evidence.
...
- saved assessment;
- export generation;
- document-only versus authority-assisted context assembly;
- authority source/snapshot persistence.
...
- an authority assertion is presented or cited as source-document evidence;
- authority-assisted retrieval occurs without recording the authority source/fields used.
...
### `docs/experiment/authority-register.md`

Record:

- available authority presets/sources;
- field meanings;
- intended role in the experiment;
- interpretative-risk classification;
...
- how database authorities support entity resolution, filtering, controlled expansion and optional context without becoming source-document evidence.
...
- GraphQL authority inventory and role mapping;
- authority/source identity links;
...
- integrate only explicit authority filters / controlled expansions;
- record authority influence on each retrieval;
- add tests.
...
An archive/database authority may identify, classify or contextualise a record, but it does not become source-document evidence by being supplied to the model.

Prefer document-only evidence for substantive historical claims. Use authority context to resolve, filter, expand or contextualise only when its role is explicit and inspectable.
...
## 30. Prohibited implementation shortcuts
...
- merge authority data into source text without labelling it;
- let authority-derived classifications validate themselves;
- use authority-assisted query expansion without storing the expansion and source;
- treat absence from an authority table as historical absence.
...
## 32. Initial Copilot task

Begin with this exact task:

> Inspect this repository against the work instruction in this file. Do not change code yet. Map the existing ingestion, PostgreSQL full-text retrieval, Granite inference, GraphQL/REST APIs, database-authority presets, archive-to-LLM evidence-surface presets, provenance handling, experiment persistence and front-end views. Then produce a gap analysis, a phased implementation plan, and a proposed list of files to create or modify. Flag any ambiguity that would affect data integrity, reproducibility or the distinction between retrieved evidence and generated inference.
