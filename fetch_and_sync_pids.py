...
def insert_pids_to_droplet(parent_records):
    ...
    for record in parent_records:
        ...
        cmd = f'ssh root@104.248.170.26 "docker exec phd-practice-db psql -U postgres -d testamentary-traces -c \\"{insert_sql}\\""'
    ...
    if data:
        ...
        if parent_records:
            ...
            result = subprocess.run(
                 'ssh root@104.248.170.26 "docker exec phd-practice-db psql -U postgres -d testamentary-traces -c \\"SELECT pid, title FROM documents WHERE pid IS NOT NULL ORDER BY pid;\\""',
                shell=True,
                capture_output=True,
                text=True
            )
