#!/usr/bin/env python3
"""Execute exactly one formal Q03 run from persisted plan KR3-v1 in production."""
...
def load_validated_plan(db) -> RetrievalPlan:
    if settings.ENVIRONMENT.lower() != "production":
        raise RuntimeError("Q03 formal execution is production-only.")

    stored_plan = db.query(TurinRetrievalPlan).filter(TurinRetrievalPlan.plan_id == PLAN_ID).one_or_none()
    if stored_plan is None:
        raise RuntimeError("Persisted plan KR3-v1 was not found.")
    ...
    if (
        ...
        raise RuntimeError("Persisted KR3-v1 does not match the approved formal Q03 configuration.")
    if not expected_case or not expected_question:
        raise RuntimeError("Registered Q03 question is unavailable.")
    plan.require_formal_approval()
    if CONTEXT_ASSEMBLY_PROTOCOL_VERSION != EXECUTION_PROTOCOL_VERSION:
        raise RuntimeError("Q03 formal execution requires the active v1.1 context allocator.")
    ...
    if protocol_registered != 1:
        raise RuntimeError("Q03 formal execution requires the registered v1.1 protocol amendment.")
    configured_corpus = "corpus_f40d78dbce52"
    if configured_corpus != CORPUS_VERSION:
        raise RuntimeError("Q03 formal execution requires the governed corpus version.")
    existing = db.execute(
    ...
    if existing != 0:
        raise RuntimeError("Q03 formal execution is blocked because a primary run already exists.")
    downstream_runs = db.execute(
        text("SELECT count(*) FROM experiment_runs WHERE question_id IN ('KR4', 'KR5', 'KR6', 'KR7', 'KR8', 'KR9', 'KR10', 'KR11', 'KR12')"),
    ).scalar_one()
    if downstream_runs != 0:
        raise RuntimeError("Q03 formal execution is blocked because later formal questions have runs.")
    return plan
...
async def main() -> None:
    ...
    try:
        ...
        if run.retrieval_plan_id != PLAN_ID or run.corpus_version != CORPUS_VERSION:
            raise RuntimeError("Persisted Q03 run does not match the required plan or corpus snapshot.")
