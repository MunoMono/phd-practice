#!/usr/bin/env python3
"""Execute the single authorized Qwen comparison of historical Q02 / KR2-v1."""
...
AUTHORIZATION_ID = "turin-qwen-q02-kr2-v1-comparison-authorization"
HISTORICAL_RUN_ID = "experiment-f3129c23fee7"
QUESTION_ID = "KR2"
PLAN_ID = "KR2-v1"
...
def preflight(db) -> RetrievalPlan:
    ...
    if historical is None or any((
        ...
        historical.corpus_version != CORPUS_VERSION, historical.model_name != "granite3.1-dense:2b-instruct-q4_K_M",
        historical.status != "completed",
    )):
        raise RuntimeError("The immutable Granite KR2-v1 baseline does not match the registered comparison identity.")
    stored_plan = db.query(TurinRetrievalPlan).filter(TurinRetrievalPlan.plan_id == PLAN_ID).one_or_none()
    if stored_plan is None:
        raise RuntimeError("KR2-v1 is not registered.")
    plan = RetrievalPlan.model_validate(stored_plan.plan_json)
    expected_facets = [
        ("person", ["Bruce Archer"]),
        ("teaching_learning", ["teaching", "learning", "student", "students", "tutorial"]),
    ]
    if any((
        plan.question_id != QUESTION_ID, plan.plan_version != "1.0", plan.top_k != 5,
        plan.retrieval_scope != "corpus_wide", plan.run_classification != "primary",
        [(facet.facet_id, facet.alternatives) for facet in plan.lexical_facets] != expected_facets,
    )):
        raise RuntimeError("KR2-v1 no longer matches the frozen comparison plan.")
    ...
    if authorization is None or any((
        authorization["question_id"] != QUESTION_ID, authorization["plan_id"] != PLAN_ID,
        authorization["plan_version"] != "1.0", authorization["execution_protocol_version"] != QWEN_COMPARISON_PROTOCOL_VERSION,
        authorization["corpus_version"] != CORPUS_VERSION, authorization["model_name"] != MODEL,
    ...
    if prior_runs != 0:
        raise RuntimeError("Qwen Q02/KR2-v1 comparison already has a persisted run; no retry is authorized.")
    return plan
    ...
    try:
        ...
        run = await ExperimentRunService(inference_service=inference_service).run_archival_experiment(
            ...
            ExperimentRunRequest(
                research_case=research_case, research_question=question, question_id=QUESTION_ID,
                retrieval=RetrievalValidationRequest(query=question, top_k=plan.top_k, corpus_version=CORPUS_VERSION),
                context_budget=6000, retrieval_plan=plan,
