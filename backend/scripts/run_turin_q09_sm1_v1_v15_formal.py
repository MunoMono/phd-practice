#!/usr/bin/env python3
"""Execute the one authorized SM1-v1 / v1.5 formal run without JSON repair."""
...
AUTHORIZATION_ID = 'turin-q09-sm1-v1-v15-primary-authorization'
...
def load_preflight(db) -> RetrievalPlan:
    ...
    if stored is None:
        raise RuntimeError('SM1-v1 is not registered.')
    ...
    if any((
        ...
        raise RuntimeError('SM1-v1/v1.5 preflight identity does not match the governed configuration.')
    ...
    if authorization is None or dict(authorization['model_parameters_json']) != EXPECTED_PARAMETERS or any((
        ...
        authorization['authorization_category'] != 'q09_sm1_v15_primary_authorization', authorization['model_name'] != 'granite3.1-dense:2b-instruct-q4_K_M',
    )):
        raise RuntimeError('Q09 requires its exact unmodified SM1-v1/v1.5 authorization.')
    for label, sql, expected in (
        ('authorization consumption', 'SELECT count(*) FROM experiment_runs WHERE formal_authorization_id=:id', 0),
        ('SM1 formal runs', "SELECT count(*) FROM experiment_runs WHERE question_id='SM1' AND fixture_only=false", 0),
        ('SM1 evaluable results', "SELECT count(*) FROM turin_formal_run_evaluability_classifications WHERE question_id='SM1'", 0),
