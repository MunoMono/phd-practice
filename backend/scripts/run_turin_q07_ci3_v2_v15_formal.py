#!/usr/bin/env python3
"""Execute the one authorized CI3-v2 / v1.5 formal run without JSON repair."""
...
AUTHORIZATION_ID = 'turin-q07-ci3-v2-v15-primary-authorization'
...
def load_preflight(db) -> RetrievalPlan:
    ...
    strata = {item.stratum_id: item for item in plan.temporal_strata or []}
    if any((
        ...
        set(strata) != {'contemporary', 'retrospective'},
        strata['contemporary'].top_k != 4, strata['contemporary'].year_to != 1985,
        strata['retrospective'].top_k != 2, strata['retrospective'].year_from != 1986,
        strata['retrospective'].source_type != 'oral_history',
        ...
        raise RuntimeError('CI3-v2/v1.5 preflight identity does not match the governed configuration.')
    ...
    if authorization is None or dict(authorization['model_parameters_json']) != EXPECTED_PARAMETERS or any((
        ...
        authorization['authorization_category'] != 'q07_ci3_v2_v15_primary_authorization', authorization['model_name'] != 'granite3.1-dense:2b-instruct-q4_K_M',
    )):
        raise RuntimeError('Q07 requires its exact unmodified v2/v1.5 authorization.')
    for label, sql, expected in (
        ('authorization consumption', 'SELECT count(*) FROM experiment_runs WHERE formal_authorization_id=:id', 0),
        ('CI3-v2/v1.5 formal runs', "SELECT count(*) FROM experiment_runs WHERE question_id='CI3' AND retrieval_protocol_version='turin-retrieval-protocol-v1.5' AND fixture_only=false", 0),
