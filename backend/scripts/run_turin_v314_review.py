...
try:
 authorities=[dict(x) for x in db.execute(text('SELECT authority_type,authority_id,code,label,metadata FROM database_authorities ORDER BY authority_type,authority_id')).mappings().all()]; selector=TurinRetrievalV314ProjectResolution(authorities)
 for display,qid in IDS:
  _,q=TURIN_QUESTION_REGISTER[qid]; r=service.retrieve(db,q,5,CORPUS_VERSION); selected=selector.build(service.last_passage_candidates,q,r['question_analysis']['required_facets']); data={'version':'v3.14-read-only','question_id':display,'question':q,'qwen_calls':0,'formal_runs':0,'query_anchors':selected['query_anchors'],'source_profiles':[{k:v for k,v in p.items() if k!='top_passages'} for p in selected['source_profiles']],'evidence_bundles':selected['evidence_bundles'],'retrieval_adequacy':selected['retrieval_adequacy']};(ROOT/'artifacts'/f'turin_v314_review_{display.lower()}.json').write_text(json.dumps(data,indent=2)+'\n');out.append(data)
finally: db.close()
(ROOT/'artifacts'/'turin_v314_review_all_12.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({'reviews':len(out),'qwen_calls':0,'formal_runs':0}))
