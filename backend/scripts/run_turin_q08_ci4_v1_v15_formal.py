#!/usr/bin/env python3
"""Execute the one authorized CI4-v1 / v1.5 formal run without JSON repair."""
...
AUTHORIZATION_ID = 'turin-q08-ci4-v1-v15-primary-authorization'
...
def load_preflight(db) -> RetrievalPlan:
    if settings.ENVIRONMENT.lower() != 'production':
        raise RuntimeError('Q08 formal execution is production-only.')
    stored = db.query(TurinRetrievalPlan).filter(TurinRetrievalPlan.plan_id == PLAN_ID).one_or_none()
    if stored is None:
        raise RuntimeError('CI4-v1 is not registered.')
    ...
    expected_facets = [
        ...
        ('systematic_process', ['systematic design', 'design process']),
    ]
    if any((
        ...
        raise RuntimeError('CI4-v1/v1.5 preflight identity does not match the governed configuration.')
    ...
    if authorization is None or dict(authorization['model_parameters_json']) != EXPECTED_PARAMETERS or any((
        ...
        authorization['authorization_category'] != 'q08_ci4_v15_primary_authorization', authorization['model_name'] != 'granite3.1-dense:2b-instruct-q4_K_M',
    )):
        raise RuntimeError('Q08 requires its exact unmodified CI4-v1/v1.5 authorization.')
    for label, sql, expected in (
        ('authorization consumption', 'SELECT count(*) FROM experiment_runs WHERE formal_authorization_id=:id', 0),
        ('CI4 formal runs', "SELECT count(*) FROM experiment_runs WHERE question_id='CI4' AND fixture_only=false", 0),
        ('CI4 evaluable results', "SELECT count(*) FROM turin_formal_run_evaluability_classifications WHERE question_id='CI4'", 0),
        ('Q09-Q12 runs', "SELECT count(*) FROM experiment_runs WHERE question_id IN ('SM1','SM2','SM3','SM4')", 0),
    ):
        if db.execute(text(sql), {'id': AUTHORIZATION_ID}).scalar_one() != expected:
            raise RuntimeError(f'Q08 execution blocked: {label} is no longer {expected}.')
    return plan
...
async def verify_write_ahead_shell(run: ExperimentRun) -> None:
    ...
    try:
        ...
        if shell is None or any((
            ...
            raise RuntimeError('Q08 write-ahead shell is not durably present with the governed identity.')
