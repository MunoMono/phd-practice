#!/usr/bin/env python3
"""Read-only V3.2 candidate and archival-context export for V3.8 bundling."""
...
OUTPUT = ROOT / "artifacts" / "turin_v38_v32_candidate_pool.json"
FIELDS = {"chunk_id", "document_id", "canonical_asset_id", "archive_record_pid", "archive_record_id", "asset_id", "asset_pid", "pid", "authority_data", "chunk_index", "chunk_text", "passage_score", "passage_adequacy", "passage_score_components", "facet_coverage", "source_type", "source_signals", "authority_graph_signals", "lane_nominations"}
...
def main():
    ...
    try:
        ...
        for case in fixture["cases"]:
            ...
            candidates = [{key: value for key, value in item.items() if key in FIELDS} for item in service.last_passage_candidates]
        ...
        OUTPUT.write_text(json.dumps({"fixture_sha256": fingerprint, "corpus_version": fixture["corpus_version"], "candidate_rule": "unchanged_V3.2_top3_per_nominated_source", "cases": cases}, indent=2) + "\n")
