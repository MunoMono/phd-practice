#!/usr/bin/env python3
"""Execute the single authorized Qwen comparison of historical Q03 / KR3-v1."""
...
AUTHORIZATION_ID = "turin-qwen-q03-kr3-v1-comparison-authorization"
HISTORICAL_RUN_ID = "experiment-af4e7f1003c9"
QUESTION_ID = "KR3"
PLAN_ID = "KR3-v1"
...
def preflight(db) -> RetrievalPlan:
    ...
    if historical is None or any((
        historical.question_id != QUESTION_ID, historical.retrieval_plan_id != PLAN_ID,
        historical.retrieval_plan_version != "1.0", historical.retrieval_protocol_version != "turin-retrieval-protocol-v1.1",
        historical.corpus_version != CORPUS_VERSION, historical.model_name != "granite3.1-dense:2b-instruct-q4_K_M", historical.status != "completed",
    )):
        raise RuntimeError("The immutable Granite KR3-v1 baseline does not match the registered comparison identity.")
    stored_plan = db.query(TurinRetrievalPlan).filter(TurinRetrievalPlan.plan_id == PLAN_ID).one_or_none()
    if stored_plan is None:
        raise RuntimeError("KR3-v1 is not registered.")
    plan = RetrievalPlan.model_validate(stored_plan.plan_json)
    expected_facets = [("contributors", ["Ken Baynes", "Phil Roberts"]), ("unit", ["Design Education Unit", "DEU"])]
    if any((plan.question_id != QUESTION_ID, plan.plan_version != "1.0", plan.top_k != 5, plan.retrieval_scope != "corpus_wide", plan.run_classification != "primary", [(facet.facet_id, facet.alternatives) for facet in plan.lexical_facets] != expected_facets)):
        raise RuntimeError("KR3-v1 no longer matches the frozen comparison plan.")
    ...
    if authorization is None or any((authorization["question_id"] != QUESTION_ID, authorization["plan_id"] != PLAN_ID, authorization["plan_version"] != "1.0", authorization["execution_protocol_version"] != QWEN_COMPARISON_PROTOCOL_VERSION, authorization["corpus_version"] != CORPUS_VERSION, authorization["model_name"] != MODEL, dict(authorization["model_parameters_json"]) != EXPECTED_PARAMETERS)):
    ...
    if prior_runs != 0:
        raise RuntimeError("Qwen Q03/KR3-v1 comparison already has a persisted run; no retry is authorized.")
    return plan
...
async def main() -> None:
    ...
    try:
        ...
        if inference_service.model_name != MODEL or inference_service.timeout_seconds != 600:
            raise RuntimeError("Qwen comparison runtime does not match its authorized configuration.")
        ...
        run = await ExperimentRunService(inference_service=inference_service).run_archival_experiment(
            db,
            ExperimentRunRequest(research_case=research_case, research_question=question, question_id=QUESTION_ID, retrieval=RetrievalValidationRequest(query=question, top_k=plan.top_k, corpus_version=CORPUS_VERSION), context_budget=6000, retrieval_plan=plan, execution_protocol_version=QWEN_COMPARISON_PROTOCOL_VERSION, formal_authorization_id=AUTHORIZATION_ID, fixture_only=False, allow_repair=False),
        )
