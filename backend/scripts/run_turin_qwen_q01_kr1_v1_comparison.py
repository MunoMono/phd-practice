#!/usr/bin/env python3
"""Execute the single authorized Qwen comparison of historical Q01 / KR1-v1."""
...
AUTHORIZATION_ID = "turin-qwen-q01-kr1-v1-comparison-authorization"
HISTORICAL_RUN_ID = "experiment-7db3ecba21e1"
QUESTION_ID = "KR1"
PLAN_ID = "KR1-v1"
...
def preflight(db) -> RetrievalPlan:
    ...
    if historical is None or any((
        ...
        historical.retrieval_plan_version != "1.0",
        historical.retrieval_protocol_version != "turin-retrieval-protocol-v1.0",
        historical.corpus_version != CORPUS_VERSION,
        historical.model_name != "granite3.1-dense:2b-instruct-q4_K_M",
        historical.status != "completed",
    )):
        raise RuntimeError("The immutable Granite KR1-v1 baseline does not match the registered comparison identity.")
    stored_plan = db.query(TurinRetrievalPlan).filter(TurinRetrievalPlan.plan_id == PLAN_ID).one_or_none()
    if stored_plan is None:
        raise RuntimeError("KR1-v1 is not registered.")
    plan = RetrievalPlan.model_validate(stored_plan.plan_json)
    expected_facets = [
        ("project_person", ["171", "Pierre Goumain"]),
    ...
    if any((
        ...
        plan.top_k != 5,
        plan.retrieval_scope != "corpus_wide",
        plan.run_classification != "primary",
        [(facet.facet_id, facet.alternatives) for facet in plan.lexical_facets] != expected_facets,
    )):
        raise RuntimeError("KR1-v1 no longer matches the frozen comparison plan.")
    ...
    if prior_runs != 0:
        raise RuntimeError("Qwen Q01/KR1-v1 comparison already has a persisted run; no retry is authorized.")
    return plan
