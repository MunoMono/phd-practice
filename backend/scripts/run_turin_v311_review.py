...
try:
    ...
    selector = TurinRetrievalV311TypedAnchors(authorities)
    for display_id, question_id in IDS:
        ...
        review = {"version": "v3.11-read-only", "question_id": display_id, "question": question, "qwen_calls": 0, "formal_runs": 0, "query_anchors": selected["query_anchors"], "source_profiles": [{key: value for key, value in profile.items() if key != "top_passages"} for profile in selected["source_profiles"]], "evidence_bundles": selected["evidence_bundles"], "retrieval_adequacy": selected["retrieval_adequacy"]}
        (ROOT / "artifacts" / f"turin_v311_review_{display_id.lower()}.json").write_text(json.dumps(review, indent=2) + "\n")
        reviews.append(review)
finally:
    db.close()
(ROOT / "artifacts" / "turin_v311_review_all_12.json").write_text(json.dumps(reviews, indent=2) + "\n")
