...
class ExperimentRunService:
    ...
    def _finalize_write_ahead_run(self, db: Any, run: ExperimentRun, inference: dict[str, Any] | None, error: tuple[str, str] | None) -> ExperimentRun:
        ...
        run.output_sha256 = experiment_run_digest(
            ...
            [item.evidence_sha256 for item in sorted(run.evidence or [], key=lambda item: item.rank)],
        )
        db.commit()
        db.refresh(run)
        return run
    ...
    def _persist(self, db: Any, request: ExperimentRunRequest, retrieval: dict[str, Any], context: Any, inference: dict[str, Any] | None, error: tuple[str, str] | None, status_override: str | None = None) -> ExperimentRun:
        ...
        for result in retrieval["results"]:
            ...
            snapshot = _snapshot(copy.deepcopy(result))
            evidence_payload = {"run_id": run_id, "rank": result["rank"], "chunk_id": result["chunk_id"], "document_id": result["document_id"], "supplied_excerpt": supplied.get(result["chunk_id"], {}).get("text"), "snapshot_json": snapshot}
            evidence_records.append(ExperimentRunEvidence(run_id=run_id, rank=result["rank"], score=result["score"], document_id=result["document_id"], pid=result.get("pid"), archive_record_pid=result.get("archive_record_pid"), archive_resolution_status=result["archive_resolution_status"], page_start=result.get("page_start"), page_end=result.get("page_end"), chunk_id=result["chunk_id"], chunk_sequence=result.get("chunk_sequence"), excerpt=result["text"], supplied_excerpt=evidence_payload["supplied_excerpt"], included_in_context=bool(decision.get("included_in_context")), original_chars=decision.get("original_chars"), supplied_chars=decision.get("supplied_chars"), excerpted=decision.get("excerpted"), exclusion_reason=decision.get("exclusion_reason"), snapshot_json=snapshot, evidence_sha256=experiment_evidence_digest(evidence_payload)))
        run.output_sha256 = experiment_run_digest({"run_id": run.run_id, "research_question": run.exact_research_question, "corpus_version": run.corpus_version, "git_commit": run.git_commit, "retrieval_config_json": run.retrieval_config_json, "model_parameters_json": run.model_parameters_json, "structured_response_json": run.structured_response_json, "provenance_validation_json": run.provenance_validation_json, "status": run.status}, [item.evidence_sha256 for item in evidence_records])
        db.add(run)
