-- One append-only authorization for the CI3-v2 / v1.5 formal execution.
BEGIN;
...
ALTER TABLE turin_formal_protocol_authorizations
    ...
    ADD CONSTRAINT turin_formal_protocol_authorizations_governed_pair_check CHECK (
        ...
        OR (execution_protocol_version = 'turin-retrieval-protocol-v1.5' AND authorization_category = 'q07_ci3_v2_v15_primary_authorization')
    );

CREATE OR REPLACE FUNCTION validate_turin_clean_primary_authorization()
RETURNS TRIGGER AS $$
DECLARE
    expected_protocol VARCHAR(255);
...
BEGIN
    IF NEW.formal_authorization_id = 'turin-q07-ci3-v2-v14-primary-authorization' THEN
        expected_protocol := 'turin-retrieval-protocol-v1.4';
        expected_authorization_category := 'q07_ci3_v2_v14_primary_authorization';
    ELSIF NEW.formal_authorization_id = 'turin-q07-ci3-v2-v15-primary-authorization' THEN
        expected_protocol := 'turin-retrieval-protocol-v1.5';
        expected_authorization_category := 'q07_ci3_v2_v15_primary_authorization';
    ELSE
        RETURN NEW;
    END IF;
    ...
    IF NEW.recovery_of_run_id IS NOT NULL
       ...
       OR NEW.retrieval_plan_id <> 'CI3-v2'
       OR NEW.retrieval_plan_version <> '2.0'
       OR NEW.retrieval_protocol_version <> expected_protocol
       ...
       OR NOT EXISTS (
        ...
        RAISE EXCEPTION 'Q07 CI3-v2 formal execution requires its exact authorization and frozen identity.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
SELECT
    'turin-q07-ci3-v2-v15-primary-authorization', NULL, 'CI3', 'CI3-v2', '2.0',
    'turin-retrieval-protocol-v1.0', 'turin-retrieval-protocol-v1.5',
    'corpus_f40d78dbce52', 'corpus_wide', 'primary',
    'q07_ci3_v2_v15_primary_authorization', 'granite3.1-dense:2b-instruct-q4_K_M',
...
WHERE EXISTS (
    ...
    WHERE plan_id = 'CI3-v2' AND question_id = 'CI3' AND plan_version = '2.0'
      AND researcher_approval_state = 'approved' AND run_classification = 'primary'
      AND plan_json->>'top_k' = '6'
      AND plan_json->'temporal_strata' = '[{"stratum_id":"contemporary","classification":"contemporary DDR document","top_k":4,"year_from":null,"year_to":1985,"source_type":"any"},{"stratum_id":"retrospective","classification":"later retrospective account","top_k":2,"year_from":1986,"year_to":null,"source_type":"oral_history"}]'::jsonb
)
AND EXISTS (
    SELECT 1 FROM turin_retrieval_protocol_amendments
    WHERE protocol_version = 'turin-retrieval-protocol-v1.5'
      AND supersedes_protocol_version = 'turin-retrieval-protocol-v1.4'
      AND amendment_scope = 'structured_response_contract_enforcement'
...
AND NOT EXISTS (SELECT 1 FROM turin_formal_protocol_authorizations WHERE authorization_id = 'turin-q07-ci3-v2-v15-primary-authorization')
AND NOT EXISTS (SELECT 1 FROM experiment_runs WHERE question_id = 'CI3' AND retrieval_protocol_version = 'turin-retrieval-protocol-v1.5' AND fixture_only = false)
AND NOT EXISTS (SELECT 1 FROM turin_formal_run_evaluability_classifications WHERE question_id = 'CI3')
AND NOT EXISTS (SELECT 1 FROM experiment_runs WHERE question_id IN ('CI4', 'SM1', 'SM2', 'SM3', 'SM4'));
