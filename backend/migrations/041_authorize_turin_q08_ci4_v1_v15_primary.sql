-- One append-only authorization for the CI4-v1 / v1.5 formal execution.
BEGIN;
...
ALTER TABLE turin_formal_protocol_authorizations
    ...
    ADD CONSTRAINT turin_formal_protocol_authorizations_governed_pair_check CHECK (
        ...
        OR (execution_protocol_version = 'turin-retrieval-protocol-v1.5' AND authorization_category IN ('q07_ci3_v2_v15_primary_authorization', 'q08_ci4_v15_primary_authorization'))
    );
...
SELECT
    'turin-q08-ci4-v1-v15-primary-authorization', NULL, 'CI4', 'CI4-v1', '1.0',
    'turin-retrieval-protocol-v1.0', 'turin-retrieval-protocol-v1.5',
    'corpus_f40d78dbce52', 'corpus_wide', 'primary',
    'q08_ci4_v15_primary_authorization', 'granite3.1-dense:2b-instruct-q4_K_M',
...
WHERE EXISTS (
    ...
    WHERE plan_id = 'CI4-v1' AND question_id = 'CI4' AND plan_version = '1.0'
      ...
      AND plan_json->'lexical_facets' = '[{"facet_id":"institution","alternatives":["Department of Design Research","DDR"]},{"facet_id":"systematic_process","alternatives":["systematic design","design process"]}]'::jsonb
)
AND EXISTS (
...
AND NOT EXISTS (SELECT 1 FROM turin_formal_protocol_authorizations WHERE authorization_id = 'turin-q08-ci4-v1-v15-primary-authorization')
AND NOT EXISTS (SELECT 1 FROM experiment_runs WHERE question_id = 'CI4' AND fixture_only = false)
AND NOT EXISTS (SELECT 1 FROM turin_formal_run_evaluability_classifications WHERE question_id = 'CI4')
AND NOT EXISTS (SELECT 1 FROM experiment_runs WHERE question_id IN ('SM1', 'SM2', 'SM3', 'SM4'));
