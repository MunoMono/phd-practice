-- One append-only authorization for the CI3-v2 / v1.4 formal execution.
BEGIN;
...
BEGIN
    IF NEW.formal_authorization_id = 'turin-q07-ci3-v2-v14-primary-authorization' THEN
        IF NEW.recovery_of_run_id IS NOT NULL
           ...
           OR NEW.retrieval_protocol_version <> 'turin-retrieval-protocol-v1.4'
           ...
           OR NOT EXISTS (
               ...
               WHERE auth_record.authorization_id = NEW.formal_authorization_id
                 ...
                 AND auth_record.authorization_category = 'q07_ci3_v2_v14_primary_authorization'
           ) THEN
            RAISE EXCEPTION 'Q07 CI3-v2/v1.4 execution requires its exact formal authorization and frozen identity.';
        END IF;
    END IF;
    RETURN NEW;
END;
...
SELECT
    'turin-q07-ci3-v2-v14-primary-authorization', NULL, 'CI3', 'CI3-v2', '2.0',
    'turin-retrieval-protocol-v1.0', 'turin-retrieval-protocol-v1.4',
    'corpus_f40d78dbce52', 'corpus_wide', 'primary',
    'q07_ci3_v2_v14_primary_authorization', 'granite3.1-dense:2b-instruct-q4_K_M',
...
WHERE EXISTS (
    ...
    AND researcher_approval_state = 'approved' AND run_classification = 'primary'
      ...
      AND plan_json->'temporal_strata' = '[{"stratum_id":"contemporary","classification":"contemporary DDR document","top_k":4,"year_to":1985,"source_type":"any"},{"stratum_id":"retrospective","classification":"later retrospective account","top_k":2,"year_from":1986,"source_type":"oral_history"}]'::jsonb
)
AND EXISTS (
    SELECT 1 FROM turin_retrieval_protocol_amendments
    WHERE protocol_version = 'turin-retrieval-protocol-v1.4'
      AND supersedes_protocol_version = 'turin-retrieval-protocol-v1.3'
      AND amendment_scope = 'structured_output_capacity'
      AND amendment_json->>'structured_output_max_tokens' = '1500'
)
AND NOT EXISTS (SELECT 1 FROM turin_formal_protocol_authorizations WHERE question_id = 'CI3' AND plan_id = 'CI3-v2')
AND NOT EXISTS (SELECT 1 FROM experiment_runs WHERE question_id = 'CI3' AND retrieval_plan_id = 'CI3-v2' AND fixture_only = false)
