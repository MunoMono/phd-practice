...
ALTER TABLE turin_formal_protocol_authorizations
    ...
    ADD CONSTRAINT turin_formal_protocol_authorizations_governed_pair_check CHECK (
        ...
        OR (execution_protocol_version = 'turin-comparison-protocol-v2.0-qwen' AND authorization_category IN ('qwen_comparison_q09_sm1_v2_single_run', 'qwen_comparison_q01_kr1_v1_single_run', 'qwen_comparison_q02_kr2_v1_single_run', 'qwen_comparison_q03_kr3_v1_single_run'))
    );
...
SELECT
    'turin-qwen-q03-kr3-v1-comparison-authorization', NULL,
    'KR3', 'KR3-v1', '1.0', 'turin-retrieval-protocol-v1.0',
    'turin-comparison-protocol-v2.0-qwen', 'corpus_f40d78dbce52', 'corpus_wide',
    'primary', 'qwen_comparison_q03_kr3_v1_single_run', 'qwen3:8b-q4_K_M',
    '{"max_tokens":1500,"temperature":0.0,"top_p":1.0,"do_sample":false,"runtime_timeout_seconds":600}'::jsonb
WHERE EXISTS (
    SELECT 1 FROM experiment_runs
    WHERE run_id = 'experiment-af4e7f1003c9'
      AND question_id = 'KR3' AND retrieval_plan_id = 'KR3-v1'
      AND retrieval_plan_version = '1.0' AND retrieval_protocol_version = 'turin-retrieval-protocol-v1.1'
      AND corpus_version = 'corpus_f40d78dbce52'
      AND model_name = 'granite3.1-dense:2b-instruct-q4_K_M' AND status = 'completed'
)
AND EXISTS (
    SELECT 1 FROM turin_retrieval_plans
    WHERE plan_id = 'KR3-v1' AND question_id = 'KR3' AND plan_version = '1.0'
      ...
      AND plan_json->'lexical_facets' = '[{"facet_id":"contributors","alternatives":["Ken Baynes","Phil Roberts"]},{"facet_id":"unit","alternatives":["Design Education Unit","DEU"]}]'::jsonb
)
AND NOT EXISTS (SELECT 1 FROM turin_formal_protocol_authorizations WHERE authorization_id = 'turin-qwen-q03-kr3-v1-comparison-authorization')
AND NOT EXISTS (SELECT 1 FROM experiment_runs WHERE retrieval_protocol_version = 'turin-comparison-protocol-v2.0-qwen' AND question_id = 'KR3' AND retrieval_plan_id = 'KR3-v1' AND fixture_only = false);
